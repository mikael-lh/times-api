...
uv run dbt build --select "state:modified+ source_status:fresher+" \
    --state ../previous-run/target/   # daily-style selective run
...
| Workflow | When | What |
|---|---|---|
| [`dbt-run.yml`](../.github/workflows/dbt-run.yml) | Daily 08:00 UTC + manual | **dbt Fusion** (`dbt==2.0.0rc194` via uv): `dbt source freshness` on `most_popular_articles` and `best_sellers`, then selective `dbt build --select state:modified+ source_status:fresher+` against cached prior artifacts (full build on first run); uploads a static docs site artifact. |
